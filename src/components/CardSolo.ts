...
export class CardSolo extends BaseCard {
    render(root: ShadowRoot | HTMLElement): HTMLElement {
        ...
        const section = createElement("section", "signature-card relative", {}, root);
    ...
    private renderHeader(parent: HTMLElement) {
        ...
        createElement("img", "", { src: SOCIAL_ICONS.certification, alt: "Vérifié" }, badge);
    ...
    private renderTabs(parent: HTMLElement) {
        ...
        const createTab = (id: string, label: string, svgPath: string) => {
            const btn = createElement("button", "tab-btn flex items-center gap-2", { type: "button", "data-tab": id }, tabsWrap);
        ...
        createTab("stack", "Stack", "M9.75 3v2.25M14.25 3v2.25M4.5 7.5h15M6.75 7.5v11.25m10.5-11.25v11.25M9 18h6");
        createTab("objectif", "Objectif", "M12 6v6l4 2m6-2a10 10 0 11-20 0 10 10 0 0120 0z");
    ...
    private renderStackPanel(parent: HTMLElement) {
        ...
        createElement("p", "section-label", {}, panel).textContent = "Technologies utilisées";
        createElement("h3", "section-title", {}, panel).textContent = "Architecture";
        ...
        const renderGroup = (title: string, items?: any[]) => {
            ...
            createElement("p", "text-sm font-semibold uppercase tracking-wide text-slate-500", {}, group).textContent = title;
            
            items.forEach(tech => {
                const item = createElement("div", "tech-item", {}, group);
                const icon = getStackIcon(tech.name);
                if (icon) createElement("img", "tech-icon", { src: icon, alt: tech.name, loading: "lazy" }, item);
                
                const details = createElement("div", "", {}, item);
                createElement("p", "tech-title", {}, details).textContent = tech.name;
                if (tech.version) createElement("p", "tech-meta", {}, details).textContent = `Version ${tech.version}`;
            });
        };
    ...
    private renderGoalPanel(parent: HTMLElement) {
        const { content } = this.options;
        const panel = createElement("div", "", { "data-panel": "objectif" }, parent);
        createElement("p", "section-label", {}, panel).textContent = "But du site";
        createElement("h3", "section-title", {}, panel).textContent = "Objectif du projet";
        createElement("p", "section-text", {}, panel).textContent = content?.projectGoal || "Aucun objectif défini.";
    }

    private renderStatsPanel(parent: HTMLElement) {
        const { content } = this.options;
        const panel = createElement("div", "", { "data-panel": "stats" }, parent);
        createElement("p", "section-label", {}, panel).textContent = "Statistiques du projet";
        ...
        content?.stats?.forEach(stat => {
            ...
            createElement("p", "stat-value", {}, left).textContent = String(stat.value);
            if (stat.description) createElement("p", "stat-meta", {}, left).textContent = stat.description;
        });
    }
    ...
    private renderSocialsBlock(parent: HTMLElement) {
        ...
        createElement("p", "section-label", {}, block).textContent = "Réseaux sociaux";
        const list = createElement("div", user.social_long ? "social-list mt-4" : "social-icons-only mt-5", {}, block);

        user.socials?.forEach(social => {
            const icon = getSocialIcon(social.name);
            if (user.social_long) {
                const item = createElement("a", "social-item", { href: social.url, target: "_blank", rel: "noopener" }, list);
                if (icon) createElement("img", "social-icon", { src: icon, alt: social.name }, item);
                const textWrap = createElement("div", "", {}, item);
                createElement("p", "social-title", {}, textWrap).textContent = social.name;
                createElement("p", "social-link", {}, textWrap).textContent = social.url;
            } else if (icon) {
                const link = createElement("a", "", { href: social.url, target: "_blank" }, list);
                createElement("img", "w-6 h-6", { src: icon, alt: social.name }, link);
            }
        });
    }
    ...
    private bindCardTabs(section: HTMLElement) {
        ...
        buttons.forEach(btn => {
            btn.addEventListener("click", () => this.activateCardTab(section, btn.dataset.tab!));
        });
    }
